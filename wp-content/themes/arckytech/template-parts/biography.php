<?php
    ...
    $author_info = get_the_author_meta( 'arckytech_write_by');
    ...
    $youtube_url = get_the_author_meta( 'arckytech_youtube' );
    $arckytech_write_by = get_the_author_meta( 'arckytech_write_by' );
    $author_bio_avatar_size = 180;
    if ( $author_data != '' ):
?>

    <div class="arc-postbox-details-author d-sm-flex align-items-start" data-bg-color="#F4F7F9">
        <div class="arc-postbox-details-author-thumb">
            <a href="<?php print esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) )?>">
                <?php print get_avatar( get_the_author_meta( 'user_email' ), $author_bio_avatar_size, '', '', [ 'class' => 'media-object img-circle' ] );?>  
            </a>
        </div>
        <div class="arc-postbox-details-author-content">
            <span><?php echo esc_html__( 'Written by', 'arckytec' ); ?></span>
            <h5 class="arc-postbox-details-author-title">
                <a href="<?php print esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) )?>"><?php print esc_html($author_info); ?></a>
            </h5>
            <p><?php the_author_meta( 'description' );?></p>

            <div class="arc-postbox-details-author-social">
